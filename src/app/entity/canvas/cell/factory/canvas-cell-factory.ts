...
import { ObjectGrid } from "../../../helper/grid/object-grid";
import { StringUtil } from "../../../helper/string/util/string-util";
import { DbColumnModel } from "../../../model/database/column/db-column-model";
import { DbForeignKeyModel } from "../../../model/database/constraint/key/foreign/db-foreign-key-model";
import { DbPrimaryKeyColumnModel } from "../../../model/database/constraint/key/primary/column/db-primary-key-column-model";
import { DbPrimaryKeyModel } from "../../../model/database/constraint/key/primary/db-primary-key-model";
import { DbModel } from "../../../model/database/db-model";
import { DbTableModel } from "../../../model/database/table/db-table-model";
...
export class CanvasCellFactory {
    ...
    if (json) {
      ...
      if (canvasCellType) {
        ...
        if (canvasCellType === "custom") {
          ...
          console.log("Unrecognized Canvas Cell Type [" + canvasCellType + "]; unable to deserialize JSON [" + json + "]");
        }

        if (canvasCell) {
          canvasCell.id = json.id;
          canvasCell.canvasId = json.canvasId;
          canvasCell.name = json.name;
          canvasCell.localName = json.localName;
        }
      }
    }
  ...
  static createDatabaseTableLocalName(dbModel: DbModel, tableModel: DbTableModel): string {
    return "Table-" + dbModel.name.getFullName() + "." + tableModel.name;
  }

  static createDatabaseLocalName(dbModel: DbModel): string {
    return "DB-" + dbModel.name.getFullName();
  }
