...
export class Canvas extends ApiEntity {
  ...
  static initializeJointCanvasValues( canvasId: number, canvasName: string, jointGraph: dia.Graph ): void {
    JointModelValueAccessor.initializeValue( CANVAS_ID_KEY, canvasId, jointGraph );
    JointModelValueAccessor.initializeValue( CANVAS_NAME_KEY, canvasName, jointGraph );
  }

  // Get Initial Joint Values
  static getInitialJointCanvasId(jointGraph: dia.Graph): number {
    return JointModelValueAccessor.getInitialValue( CANVAS_ID_KEY, jointGraph );
  }
  static getInitialJointCanvasName(jointGraph: dia.Graph): string {
    return JointModelValueAccessor.getInitialValue( CANVAS_NAME_KEY, jointGraph );
  }

  // Get Current Joint Values
  static getCurrentJointCanvasId(jointGraph: dia.Graph): number {
    return JointModelValueAccessor.getCurrentValue( CANVAS_ID_KEY, jointGraph );
  }
  static getCurrentJointCanvasName(jointGraph: dia.Graph): string {
    return JointModelValueAccessor.getCurrentValue( CANVAS_NAME_KEY, jointGraph );
  }

  // Set Joint Values
  static setJointCanvasId(canvasId: string, jointGraph: dia.Graph): void {
    JointModelValueAccessor.setCurrentValue( CANVAS_ID_KEY, canvasId, jointGraph );
  }
